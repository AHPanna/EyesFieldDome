...
class NotificationService:
    ...
    async def send_email(user: User, subject: str, content: str, db: Session, alert_id: int, slot_id: int) -> bool:
        ...
        if success:
            ...
            db.add(notif)
            if cost > 0:
                db.add(CreditLog(user_id=user.id, amount=-cost, operation=f"Notification Email (Alerte #{alert_id})"))
            db.add(user)
            db.commit()
            return True
        return False
    ...
    async def send_sms(user: User, message: str, db: Session, alert_id: int, slot_id: int) -> bool:
        ...
        if success:
            ...
            db.add(notif)
            db.add(CreditLog(user_id=user.id, amount=-cost, operation=f"Notification SMS (Alerte #{alert_id})"))
            db.add(user)
            db.commit()
            return True
        return False
